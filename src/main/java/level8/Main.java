            ...
            switch (command) {
                ...
                default:
                    if (command.startsWith("삭제")) {
                        ...
                        int id = Integer.parseInt(parts[1]);
                        if(map.containsKey(id)){
                            System.out.println("명언(기존) : " + map.get(id).wisesaying);
                            System.out.print("명언 : ");
                            String newQuote = scanner.nextLine();
                            System.out.println("작가(기존) : " + map.get(id).wisesayingAuthor);
                            System.out.print("작가 : ");
                            String newAuthor = scanner.nextLine();
                            Wisesaying newWise = new Wisesaying(newQuote, newAuthor);
                            map.put(id, newWise);
                        } else{
                            System.out.println(id + "번 명언은 존재하지 않습니다.");
                        }
